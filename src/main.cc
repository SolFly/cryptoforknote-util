...
NAN_METHOD(fill_extra) {
    if (info.Length() < 3) return THROW_ERROR_EXCEPTION("You must provide three arguments (parentBlock, cnBlobType, childBlock).");

    Local<Object> target = info[0]->ToObject();
    if (!Buffer::HasInstance(target)) return THROW_ERROR_EXCEPTION("First argument should be a buffer object.");

    if (!info[1]->IsNumber()) return THROW_ERROR_EXCEPTION("Second argument should be the blob type number");
    enum BLOB_TYPE blob_type = static_cast<enum BLOB_TYPE>(Nan::To<int>(info[1]).FromMaybe(0));
